...
from hint.falkorDB.HintFalkorDB import *
import os
import sys
...
class FalkorDBTester(TesterAbs):
    ...
    def single_file_testing(self, logfile):
        ...
        logger = new_logger("logs/falkordb.log", True)
        qt = QueryTransformer()
        conf = TestConfig(
            # client=Redis("redis-stack:6379", self.database),
            client=Falkor(config.get("falkordb", "port"), self.database),
            # client=Redis(uri="localhost", database=self.database),
            logger=logger,
            hintDB = HintFalkorDB(),
            source_file=logfile,
            logic_inconsistency_trace_file='logs/falkordb_logic_error.tsv',
            database_name='falkordb',
            query_producer_func=query_producer,
            oracle_func=oracle,
            report_token=config.get('lark', 'falkordb')
        )
    ...
    def single_file_testing_alt(self, logfile, create_statements, query_statements):
        ...
        logger = new_logger("logs/falkor.log")
        qt = QueryTransformer()
        conf = TestConfig(
            # client=Redis("grev-redis-stack-1:6379", self.database),
            
            client=Redis(config.get("falkordb", "port"), self.database),
            # client=Redis(uri="localhost", database=self.database),
            logger=logger,
            source_file=logfile,
            logic_inconsistency_trace_file='logs/falkordb_logic_error.tsv',
...
def schedule():
    scheduler(config.get('falkordb', 'input_path'), FalkorDBTester(f"falkor_misc"), "falkordb")
